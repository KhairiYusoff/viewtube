...
## Project bootstrap and dev startup

From the repo root:

- If no `package.json` exists, initialize the app:
...
Confirm the project scaffold by checking the repo root for `package.json`, `next.config.js`, and `src/app/page.tsx` before running the dev server.
